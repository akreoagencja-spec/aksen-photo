...
export default async function HomePage() {
  ...
  return <main>
    ...
    <section className="section muted-section"><div className="shell"><div className="section-head"><p className="eyebrow">Portfolio ślubne</p><h2>Pełne reportaże. Od pierwszego kadru do ostatniego tańca.</h2><p>To tutaj widać, jak naprawdę pracuję: przygotowania, ceremonia, bliscy, detale, emocje i wesele.</p></div>{featured.length ? <div className="story-grid">{featured.map((item,index)=><ReportageCard key={item.id} item={item} featured={index===0}/>)}</div> : <div className="empty-state">Reportaże pojawią się automatycznie z obecnego WordPressa.</div>}</div></section>
